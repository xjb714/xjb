    ...
    if (p >= -3 && p <= 6) {
        ...
        if (dot_pos <= 0) {
            ...
            *out++ = '0';
            ...
            out += dot_pos;
            *out++ = '.';
            memcpy(out, digits + dot_pos, n - dot_pos);
            out += n - dot_pos;
        }
    } else {
        // exponential
...
int main() {
    ...
    for (unsigned i = 0; i < num_threads; ++i) {
        ...
        threads[i] = std::thread([i, begin, count, &num_processed_floats, &num_errors, percent, num_floats] {
            char actual[64] = {};
            char expected[64] = {};
            ...
            for (uint32_t j = 0; j < count; ++j) {
                ...
                if (batch >= (1u << 21) || j == count - 1) {
                    ...
                    if (i == 0 && now - last_time >= std::chrono::seconds(1)) {
                        last_time = now;
                        fprintf(stderr, "\rProgress: %5.2f%%", num_processed_floats.load() * percent);
                        fflush(stderr);
                    }
                }
                ...
                *aend = '\0';
                ...
                if (value == 0.0f) {
                    ...
                    auto dec = jkj::dragonbox::to_decimal(std::fabs(value), jkj::dragonbox::policy::sign::ignore);
                    char* p = expected;
                    if (value < 0)
                        *p++ = '-';
    ...
    fprintf(stderr, "\nTested %llu values in %.2f seconds, %llu error(s)\n", num_processed_floats.load(),
            std::chrono::duration_cast<seconds>(finish - start).count(), num_errors.load());
