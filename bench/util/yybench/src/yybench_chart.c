...
#define ARR_ADD(arr, value, type) (yy_buf_append(&(arr), (u8 *)&(value), sizeof(type)))
...
#define STR_COPY(name) do { \
    ...
    if (src->categories) {
        ...
        if (count) {
            dst->categories = (const char**)calloc(count + 1, sizeof(char *));
...
yy_chart *yy_chart_new(void) {
    yy_chart *chart = (yy_chart *)calloc(1, sizeof(struct yy_chart));
...
yy_report *yy_report_new(void) {
    yy_report *report = (yy_report *)calloc(1, sizeof(yy_report));
    return report;
}
