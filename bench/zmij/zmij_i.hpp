...
static inline void zmij_f32_to_dec(float v, unsigned int *dec_return, int *e10_return)
{
    ...
    constexpr uint64_t threshold = 1e8;
    if (bin_exp == 0 || bin_exp == traits::exp_mask) [[ZMIJ_UNLIKELY]]
    {
        ...
        dec = to_decimal_schubfach(bin_sig, 1 - 150, true);
    ...
    {
        dec = ::to_decimal<float>(bin_sig | (1 << 23), bin_exp,
                                  bin_sig != 0);
    }
    *dec_return = dec.sig * 10 + dec.last_digit;
    *e10_return = dec.exp;
