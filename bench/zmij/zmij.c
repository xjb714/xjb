...
static char* write(char* buffer, uint64_t dec_sig, int dec_exp) {
...
char* zmij_dtoa(double value, char* buffer) {
  ...
  *buffer = '-';
  ...
  if (regular) {
    ...
    if (r.lo != ((uint64_t)1 << 63) && rem10 != half_ulp && ten - upper > 1) {
      ...
      return write(buffer, ((half_ulp >= rem10) + round) ? shorter : longer, dec_exp);
    }
  }
  ...
  if ((shorter << 2) >= lower) {
    return write(buffer, shorter, dec_exp);
  }
  ...
  return write(buffer, (under_closer && under_in) ? under : over, dec_exp);
}