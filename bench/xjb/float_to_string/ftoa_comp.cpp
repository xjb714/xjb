...
static inline constexpr uint64_t umul128_hi64_fallback_xjb_comp(uint64_t x, uint64_t y)
{
...
static inline constexpr void umul128_hi64_lo64_fallback_xjb_comp(uint64_t x, uint64_t y, uint64_t &hi, uint64_t &lo)
{
	// constexpr function for all compiler and hardware. just for const value precalc , not a Runtime function.
	uint64_t a = x >> 32;
	uint64_t b = uint32_t(x);
...
static inline void mul_u128_u64_high128_xjb_comp(
	uint64_t a_high,
	uint64_t a_low,
	uint64_t b,
...
#else
	umul128_hi64_lo64_fallback_xjb_comp(a_low, b, low_high, low_low);
	umul128_hi64_lo64_fallback_xjb_comp(a_high, b, high_high, high_low);
	*result_mid = low_high + high_low;
	*result_high = high_high + (*result_mid < low_high);
#endif
}

static inline void mul_u64_u64_high128_xjb_comp(
	uint64_t a,
	uint64_t b,
	uint64_t *high,
	uint64_t *low)
{
	...
	*low = (uint64_t)hi128;
#else
	umul128_hi64_lo64_fallback_xjb_comp(a, b, *high, *low);
#endif
}

static inline int u64_lz_bits_xjb_comp(uint64_t x)
{
...
static inline int u64_tz_bits_xjb_comp(uint64_t x)
{
...
static inline uint64_t umul128_hi64_xjb_comp(uint64_t a, uint64_t b)
{
	...
	return ((u128)a * b) >> 64;
#else
	uint64_t hi = umul128_hi64_fallback_xjb_comp(a, b);
	return hi;
#endif
}

static inline uint64_t u128_madd_hi64_xjb_comp(uint64_t a, uint64_t b, uint64_t c)
{
...
#else
	uint64_t hi, lo;
	umul128_hi64_lo64_fallback_xjb_comp(a, b, hi, lo);
	return hi + ((lo + c) < c);
#endif
}
static inline uint64_t is_little_endian_xjb_comp()
{
	const int n = 1;
...
struct const_value_float_xjb_comp
{
	uint64_t c1;
	uint64_t div10000;
	uint64_t m;
...
static const struct const_value_double_xjb_comp constants_double_xjb_comp = {
	.c1 = 78913ull << (64 - 18),
	.c2 = (uint64_t)-217707,
	.c3 = (uint64_t)1e15 - 1,
...
static const struct const_value_float_xjb_comp constants_float_xjb_comp = {
...
static inline uint64_t byteswap64_xjb_comp(uint64_t x)
{
...
static inline uint64_t cmov_branchless_xjb_comp(uint64_t condition, uint64_t true_value, uint64_t false_value)
{
...
static inline uint64_t compute_double_dec_sig_len_xjb_comp(uint64_t up_down, int tz, uint64_t D17)
{
	// return (15 + D17) + cmov_branchless_xjb_comp(up_down, -1 - tz, up_down);
	return cmov_branchless_xjb_comp(up_down, (NOT_REMOVE_FIRST_ZERO ? 14 + D17 : 15) - (tz), 15 + D17);
	// return cmov_branchless_xjb_comp(up_down, 15 - tz, 15 + D17);
}
static inline uint64_t compute_double_dec_sig_len_xjb_comp_sse2(uint64_t up_down, int tz_add_48, uint64_t D17)
{
	return cmov_branchless_xjb_comp(up_down, (NOT_REMOVE_FIRST_ZERO ? 14 + D17 : 15) + 48 - tz_add_48, 15 + D17);
}
static inline uint64_t compute_float_dec_sig_len_xjb_comp(uint64_t up_down, int tz, uint64_t lz)
{
	return cmov_branchless_xjb_comp(up_down, (7 - lz) - tz, 8 - lz);
}

static inline shortest_ascii16_xjb_comp to_ascii16_xjb_comp(char *buf, const uint64_t m, const uint64_t up_down, const uint64_t D17, const struct const_value_double_xjb_comp *cv)
{
	...
	uint32_t abcdefgh = umul128_hi64_xjb_comp(m, cv->mul_const) >> (90 - 64);
	...
	int tz = u64_lz_bits_xjb_comp(zeroes) >> 2;
	return {ascii16, cmov_branchless_xjb_comp(up_down, NOT_REMOVE_FIRST_ZERO ? (14 + D17) - (tz) : 15 - tz, 15 + D17)};
	// return {ascii16 , cmov_branchless_xjb_comp(up_down, NOT_REMOVE_FIRST_ZERO ? (-1 ) - (tz) : 15 - tz , 0 ) + (15 + D17) };
	// return {ascii16 , (15 + D17) - (up_down ? (tz+1) : up_down)};
	// return {ascii16, compute_double_dec_sig_len_xjb_comp(up_down, tz, D17)};
	...
	int tz = u64_lz_bits_xjb_comp(mask);
	return {little_endian_ascii, compute_double_dec_sig_len_xjb_comp_sse2(up_down, tz, D17)};
...
// 	return {ascii16, compute_double_dec_sig_len_xjb_comp_sse2(up_down, tz, D17)};
#else // sse2
	...
	int abcdefgh_tz = u64_tz_bits_xjb_comp(a_b_c_d_e_f_g_h);
	int ijklmnop_tz = u64_tz_bits_xjb_comp(i_j_k_l_m_n_o_p);
	uint64_t abcdefgh_bcd = is_little_endian_xjb_comp() ? byteswap64_xjb_comp(a_b_c_d_e_f_g_h) : a_b_c_d_e_f_g_h;
	uint64_t ijklmnop_bcd = is_little_endian_xjb_comp() ? byteswap64_xjb_comp(i_j_k_l_m_n_o_p) : i_j_k_l_m_n_o_p;
	...
	return {abcdefgh_bcd | ZERO, ijklmnop_bcd | ZERO, compute_double_dec_sig_len_xjb_comp(up_down, tz, D17)};
#endif
}

static inline shortest_ascii8_xjb_comp to_ascii8_xjb_comp(const uint64_t m, const uint32_t up_down, uint32_t &lz, const struct const_value_float_xjb_comp *c = nullptr)
{
	...
	u64 abcdefgh_BCD = byteswap64_xjb_comp(vget_lane_u64(BCD_big_endian, 0)); // big_endian to little_endian , reverse 8 bytes
#endif
...
#endif
	// if(exp == 0) [[unlikely]]
	// 	lz = u64_lz_bits_xjb_comp(hgfedcba_BCD | 1) >> 3; // lz_max = 7;
	//lz = u64_tz_bits_xjb_comp(abcdefgh_BCD) >> 3;
	int tz = u64_lz_bits_xjb_comp(abcdefgh_BCD) >> 3;
	abcdefgh_BCD = abcdefgh_BCD >> (lz << 3);
	// abcdefgh_BCD = abcdefgh_BCD >> (u64_lz_bits_xjb_comp(hgfedcba_BCD) & 0b1111000);

	abcdefgh_BCD = is_little_endian_xjb_comp() ? abcdefgh_BCD : byteswap64_xjb_comp(abcdefgh_BCD);
	// return {abcdefgh_BCD | ZERO, compute_float_dec_sig_len_xjb_comp(up_down, tz, lz)};

	//when m = 0, abcdefgh_BCD = 0, tz = u64_lz_bits_xjb_comp(0) >> 3, this is an UB, but up_down is always 0, so we can dont care about it.
	//when up_down = 0, the tz is not used. so next line is always correct.
	return {abcdefgh_BCD | ZERO, cmov_branchless_xjb_comp(up_down, (7 ^ lz) - tz, 8 - lz)};// 7^lz == 7-lz
}
...
namespace xjb_comp
{
    static inline u64 calc_pow5_rlz(unsigned int pow5_offset)
    {
        ...
        u64 clz = u64_lz_bits_xjb_comp(p5);
	...
	char *xjb64_comp(double v, char *buf)
	{
		...
		const struct const_value_double_xjb_comp *cv = &constants_double_xjb_comp;
		u64 vi;
...
#define use_fast_path_for_integer_xjb 0
...
#if is_real_gcc
		if (nq <= u64_tz_bits_xjb_comp(c)) [[unlikely]] // use unlikely will generate jmp instruction
#else
		if (nq <= u64_tz_bits_xjb_comp(c)) //[[unlikely]]
#endif
		...
		i64 k;
		const int offset = 6;//  offset in range [3,10] has same result.
		...
		// 		u64 pow10_lo = p10[1];
		// 		u64 hi64, lo64;
		// 		mul_u128_u64_high128_xjb_comp(pow10_hi, pow10_lo, cb, &hi64, &lo64);
		...
		// 		// memcpy(buf, "0000", 4);
		// 		shortest_ascii16_xjb_comp s = to_ascii16_xjb_comp(mr, up_down, D17, cv);
		// 		i64 e10 = k + (15 + D17);
		// #ifdef __amd64__
		...
		// 		u64 one = (u128_madd_hi64_xjb_comp(dot_one, 10, offset_num)) + (u64)('0' + '0' * 256);
		// 		if (irregular) [[unlikely]]
		// 			if ((((dot_one >> 54) * 5) & ((1 << 9) - 1)) > (((half_ulp >> 55) * 5)))
		// 				one = (((dot_one >> 54) * 5) >> 9) + 1 + (u64)('0' + '0' * 256);
		// #else
		// 		// u64 one = ((dot_one * (u128)10 + cv->c4) >> 64) + (u64)('0' + '0' * 256);
		// 		u64 one = (u128_madd_hi64_xjb_comp(dot_one, 10, cv->c4)) + (u64)('0' + '0' * 256);
		// 		if (dot_one == (1ull << 62)) [[unlikely]]
		// 			one = (u64)('2' + '0' * 256);
		...
		// memcpy(buf, "00000000", 8);
#define direct_via_lut_get_pow5 0
...
#endif
            u64 cb = c << (h + 1 + offset);
            mul_u64_u64_high128_xjb_comp(cb, pow10_hi, &hi64, &lo64);
        }
        else
        {
...
#if direct_via_lut_get_pow5
            u64 pow5_rlz_v = pow5_rlz_ptr[pow5_offset]; // (5**pow5_offset) << clz(5**pow5_offset)
#else
            u64 pow5_rlz_v = calc_pow5_rlz(pow5_offset);
#endif
            u64 pow10_lo;
            mul_u128_u64_high128_xjb_comp(pow10_base_high, pow10_base_low, pow5_rlz_v, &pow10_hi, &pow10_lo);
            int alpha = ((((base + base_start) * ratio) * 217707) >> 16) + ((pow5_offset * 217707) >> 16);
            h = 1 + alpha + q;
            u64 cb = c << (h + offset + 1);
            mul_u128_u64_high128_xjb_comp(pow10_hi, pow10_lo, cb, &hi64, &lo64);
        }
        u64 dot_one = (hi64 << (64 - offset)) | (lo64 >> offset); // == floor(2**64*n)
        u64 half_ulp = (pow10_hi >> (-h) ) + ((c + 1) & 1);
        //u64 half_ulp = (h > 0 ? pow10_hi * 2 : (pow10_hi >> (-h))) + ((c + 1) & 1);
        u64 up = (half_ulp > ~0 - dot_one);
        //u64 down = ((half_ulp >> (1 - regular)) > dot_one);
        u64 down = ((half_ulp) > dot_one);
        u64 m_up = (hi64 >> offset) + up;
        u64 up_down = up + down;
        u64 one = u128_madd_hi64_xjb_comp(dot_one, 10, cv->c4); //+ (u64)('0' + '0' * 256);
        if(irregular)
        {
            ...
            u64 pow10_hi_div10 = k_regular != k ? ((pow10_hi >> 3) + (pow10_hi >> 1)) : pow10_hi;
            pow10_hi = pow10_hi_div10 << u64_lz_bits_xjb_comp(pow10_hi_div10);
			h = q + ((k * -217707 - 217707) >> 16);
			u64 half_ulp = pow10_hi >> (-h);
			u64 dot_one = (pow10_hi << (53 + h));
			u64 up = (half_ulp > ~0 - dot_one);
			u64 down = ((half_ulp >> 1) > dot_one);
			m_up = (pow10_hi >> (11 - h + (ieee_exponent - 1075 == 0))) + up;
			up_down = up + down;
        ...
        u64 D17 = (m_up >= (u64)1e15);
        //u64 D17 = m_up > (u64)cv->c3;
        u64 mr = D17 ? m_up : m_up * 10;
        shortest_ascii16_xjb_comp s = to_ascii16_xjb_comp(buf, NOT_REMOVE_FIRST_ZERO ? m_up : mr, up_down, D17, cv);
        i64 e10 = k + (15 + D17);
        ...
        //u64 exp_pos = (e10_DN <= e10 && e10 <= -1) ? s.dec_sig_len : (0 <= e10 && e10 <= e10_UP ? (e10 + 3 > s.dec_sig_len + 1 ? e10 + 3 : s.dec_sig_len + 1) : (s.dec_sig_len + 1 - (s.dec_sig_len == 1)));
        char *buf_origin = buf;
		buf += first_sig_pos;
	...
	{
		const struct const_value_float_xjb_comp *c = &constants_float_xjb_comp;
		//const struct const_value_float_xjb_comp *c = &float_table.constants_float_xjb_comp;
		//const struct float_table_comp_t *t = &float_table_comp;
		//const struct const_value_float_xjb_comp *c = &t->constants_float_xjb_comp;
...
// #endif
		...
		u64 hi64 = umul128_hi64_xjb_comp(cb, pow10_hi);
		...
		u32 lz = (m_up < (u32)1e7) + (m_up < (u32)1e6); // 0, 1, 2
		...
		shortest_ascii8_xjb_comp s = to_ascii8_xjb_comp(m_up, up_down, lz, c);
		i64 e10 = k + (8 - lz);
		...
		// // one = cmov_branchless_xjb_comp(up_down, '0' + '0' * 256, one); // prevent gcc generate branch instruction
		// if (irregular) [[unlikely]]
		// {
		// 	if ((exp_bin == 31 - 150) | (exp_bin == 214 - 150) | (exp_bin == 217 - 150)) // branch instruction
		// 		++one;
		...
		if (exp == 0) [[unlikely]]
			...
			if (m_up < 100000) [[unlikely]]
			{
				...
				u64 u;
				memcpy(&u, &buf[2], 8);
				u = is_little_endian_xjb_comp() ? u : byteswap64_xjb_comp(u);
				lz = u64_tz_bits_xjb_comp(u & 0x0f0f0f0f0f0f0f0f) / 8;
				lz += 2;
				e10 -= lz - 1;
				buf[0] = buf[lz];
		...
		// u64 exp_result_u64 = is_little_endian_xjb_comp() ? exp_result_u32 : (u64)exp_result_u32 << 32;
		// buf += exp_pos;
