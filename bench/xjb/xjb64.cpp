#include <stdint.h>
struct FloatingDecimal64_xjb64 {
    uint64_t digits; // num_digits <= 17
    int32_t exponent;
};
// static inline void xjb64(uint64_t ieee_significand, uint64_t ieee_exponent, uint64_t* sig_dec, int* exp_dec)
// {
...
//     //u64 irregular = (c == 0x10000000000000ull);
//     u64 irregular = ieee_significand == 0;
//     if ((!irregular)) [[likely]]
//         k = (q * 315653) >> 20; // branch
//     else
...
//     int h1 = q + (((-1 - k) * 217707) >> 16) + 1 + offset; // [-3,0] + offset
...
//     u64 *sig = (u64 *)&pow10_ptr[(-1 - k) * 2];
//     u64 p10_hi = sig[0];// high 64 bit
//     u64 p10_lo = sig[1];// low 64 bit
//     // u128 * u64 = u192 ----> get high 128 bit
//     u64 cb = c << (h1); // h1 = [0,3] ; 0 <= h1 <= 3;
//     u64 sig_hi = (cb * (u128)p10_lo) >> 64;
//     u64 sig_lo = (cb * (u128)p10_hi + sig_hi);
//     sig_hi = (cb * (u128)p10_hi + sig_hi) >> 64;
...
//     u64 dot_one = (sig_hi << (64 - offset)) + (sig_lo >> offset);// 64 bit
//     //u64 dot_one61 = dot_one >> 3;

//     u64 one11 = sig_hi & maskL;
//     u64 one64 = sig_lo;
//     u64 one64_hi = (one64 * (u128)10) >> 64;
//     u64 one64_lo =  one64 * (u128)10 ;// even
//     // u64 one_t = ( one11 * 10 + one64_hi + (1<< (offset-1)) ) >> offset;
//     // if (( (one11 * 10) & ((2<<offset) - 1)) + one64_lo == (1 << (offset-1))   )[[unlikely]]
//     //     one_t--;
//     //if(( (one11 * 10) & ((2<<offset) - 1)) == (1<<(offset-1)) )[[unlikely]]
//     // if( (one11 * 10) << (64-offset-1) == (1ull << 62) )[[unlikely]]
//     // {
//     //     if(one64_lo==0)one_t--;
//     // }
//     //if (( (one11 * 10) & ((2<<offset) - 1)) == (1<<9) > one64_lo  )[[unlikely]]
    
//     //const u64 r1 = (1 << (offset-1) ) ^ 0;

//     //u64 one_t = (( one11 * 10 + one64_hi + (1<< (offset-1)) - ( ( ((one11 * 10) & ((2<<offset) - 1)) - one64_lo == (1<< (offset-1)))  ) )) >> offset;
//     u64 one_t = (( one11 * 10 + one64_hi + (1<< (offset-1)) - ( ( ((one11 * 10) & ((2<<offset) - 1)) + one64_lo == (1<< (offset-1)))  ) )) >> offset;
//     //u64 one_t = (( one11 * 10 + one64_hi + 512 - ( ( ((one11 * 10) & ((2<<offset) - 1)) == (1<<9)) & (one64_lo == 0) ) )) >> 10;
//     //u64 one_t = (( one11 * 10 + one64_hi + 512 - ( ( ((one11 * 10) & ((2<<offset) - 1)) | one64_lo  == (1<<9)))) ) >> 10;
//     //printf(" one11 = %llx,one64_hi=%llx,one64_lo=%llx\n",one11,one64_hi,one64_lo);
//     //if()

//     // 10bit + 64bit : 74bit

//     u64 one , one_hi,one_lo,one_hi_even;
//     one = one_t;
//     // u64 one = (((dot_one ) * (u128)10) >> 64) + (((u64)((dot_one ) * (u128)10)) >> 63);
//     {
//         //round to nearest
//         //one_hi = (dot_one * (u128)10) >> 64;
//         //one_lo =  dot_one * (u128)10 ;// even
//         //one_hi_even = 1 - (one_hi & 1);
//         //u64 rest = ((sig_lo & maskL) * 10) >> offset; // 1.3076622631878654e65
//         //u64 rest = ((sig_lo & maskL) * 5) >> (offset - 1);
//         //u64 rest = ((sig_lo & maskL) + ((sig_lo & maskL)<<2) ) >> (offset - 1);
//         //u64 rest = ((sig_lo & maskL) + ((sig_lo<<2) & ((1<<(offset+2))-1))  ) >> (offset - 1);//5x = x + 4x
//         //one = one_hi + ((one_lo - (1 - (one_hi & 1))) >> 63);// one_lo = 0 , 
//         //one = one_hi + (( one_lo + (one_hi & 1) - 1) >> 63);
        
//         //one = one_hi + (( one_lo + rest - 1 + ( (one_hi & 1) ) ) >> 63);
        
//         //round to even
//         //if( 0 )
//         // if (one_lo + rest == 1ull << 63) [[unlikely]] // generate branch instruction
//         //     one -= !(one_hi & 1);      // if(even) one--;
...
//         //     };
//         //     for(int i=0;i<26;++i){
//         //         one+=(*(short*)&q==special_exp[i]);
//         //     }
//         // }
//         // if(0)
//         // if(irregular) [[unlikely]]
//         // {
//         //     // n-[10n]/10 > 2**(q-2)*10**(-k-1)
//         //     // 10n-[10n] > 2**(q-2)*10**(-k-1)*10
//         //     if( (u64)((dot_one) * (u128)10) > half_ulp * 5 && half_ulp < ( (u64)(-1) / 5) )
//         //     {
//         //         one = (((dot_one) * (u128)10) >> 64) + 1;
//         //     }
//         // }
//     }
//     u64 even = (c + 1) & 1;
...
static inline void xjb64_v2(uint64_t ieee_significand, uint64_t ieee_exponent, uint64_t* sig_dec, int* exp_dec)
{
    typedef __uint128_t u128;
    typedef uint64_t u64;
    ...
    u128 hi128 = (cb * sig[0] + ((cb * sig[1]) >> 64)); // sig[1] : low 64bit
    u64 dot_one = hi128 >> offset;   // == floor(2**64*n)
    u64 hi64 = hi128 >> 64;
    // select  from  {ten + 0, ten + floor(10n) , ten + floor(10n) + 1 , ten + 10}
    u64 ten = (hi64 >> offset) * 10; // == 10*m
    u64 half_ulp = sig[0] >> (-h);   // -h ---> range [1,4]
