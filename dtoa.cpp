...
u64 calc_f64_exp(int e10){
    ...
    // we use lookup table may faster;
    u32 a,bc,b,c;
    u32 neg = e10<0;
    e10 = neg ? -e10 : e10;
    u64 e = neg ? ('e' + '-' * 256) : ('e' + '+' * 256) ;
    a = ((u32)e10 * 656) >> 16; /* exp / 100 */
    bc = (u32)e10 - a * 100;    /* exp % 100 */
    b = (bc * 103) >> 10;
    c = bc - b * 10;
    u64 a_ASCII = e10>=100 ? (a + '0') : 0; 
    // u64 b_ASCII = e10>=10 ? (e10>=100 ? (b + '0')*256 : 0) : 0;
    // u64 c_ASCII = c+'0';
    u64 bc_ASCII = e10>=100 ? ((b+'0') + (c+'0')*256)*256 : ((b+'0') + (c+'0')*256);
    u64 exp_result = e + ((a_ASCII + bc_ASCII) << 16);
    return exp_result;
}
void byte_move_16(void * dst,void* src){
    // move 16byte from src to dst;
    *(((u64*)dst) + 0) = *(((u64*)src) + 0);
    *(((u64*)dst) + 1) = *(((u64*)src) + 1);
}void byte_move_8(void * dst,void* src){
...
u64 encode_16digit(u64 x, u64* hi,u64* lo){
    const u64 ZERO = 0x30303030ull + (0x30303030ull << 32);
    //x = x>=(u64)1e15 ? x : x*10;
    u64 aabbccddeeffgghh = x;
    u64 aabbccdd = x / 100000000;
    u64 eeffgghh = x - aabbccdd * 100000000;
    u64 aabb = (aabbccdd * 109951163) >> 40;//div 1e4
    u64 ccdd = aabbccdd - aabb * 10000;
    u64 eeff = (eeffgghh * 109951163) >> 40;//div 1e4
    u64 gghh = eeffgghh - eeff * 10000;
    u64 aabbccdd_BCD = encode_ten_thousands(aabb,ccdd);
    u64 eeffgghh_BCD = encode_ten_thousands(eeff,gghh);
    u64 aabbccdd_tz = __builtin_clzll(aabbccdd_BCD) / 8;//aabbccdd_BCD !=0
    u64 eeffgghh_tz = __builtin_clzll(eeffgghh_BCD) / 8;
    u64 tz = (eeffgghh_BCD == 0) ? 8 | aabbccdd_tz : eeffgghh_tz;
...
char* dtoa_xjb64(double v,char* buf){
    ...
    *(u64*)buf = *(u64*)"0.00000";
    ...
    const u64 *pow10_ptr = g + 293 * 2;
    {
        ...
        dec_sig_len = 16 + D17 - tz;
        one_ASCII = one + '0' + ('0' * 256);
        // write dec to tmp buf;
        *(u64*)&buf_dec[0] = hi;
        *(u64*)&buf_dec[8] = lo;
        *(u64*)&buf_dec[16] = one_ASCII;
    }
    //when -3<=e10 && e10 <= 15 ; we use %lf format print float number
    const int e10_LOW=-3;
    const int e10_UP=15;

    //we use look  up table to optimize the performance 
    u64 first_sig_pos = (e10_LOW<=e10 && e10<=-1) ? 1 - e10 : 0 ;
    byte_move_16(&buf[first_sig_pos] , &buf_dec[1 - D17]);
    u64 dot_pos = ( 1 <= e10 && e10<= e10_UP ) ? 1 + e10 : 1 ;
    buf[dot_pos] = '.';
    u64 second_sig_pos = (e10_LOW <= e10 && e10 <= -1)? 2 - e10 : dot_pos + 1;
    u64 buf_dec_second_pos = ( 1<=e10 && e10<=e10_UP ) ? e10 + 1 : 1;
    byte_move_16(&buf[second_sig_pos] , &buf_dec[1 - D17 + buf_dec_second_pos]);
    u64 exp_pos = (e10_LOW<=e10 && e10<=-1) ? first_sig_pos + dec_sig_len : 
    (( 0<=e10 && e10<=e10_UP ) ? (dot_pos + 2 > dec_sig_len + 1 ? dot_pos + 2 : dec_sig_len + 1) : 
    dec_sig_len + 1 - (dec_sig_len == dot_pos));
    
    if(m < (u64)1e14 )[[unlikely]]
    {
        // some subnormal number : range [5e-324,1e-309)
        const u64 BCD_MASK = 0x0F0F0F0Full + (0x0F0F0F0Full<<32);
        u64 aabbccdd_BCD = hi & BCD_MASK;
        u64 eeffgghh_BCD = lo & BCD_MASK;
        u64 hi_lz = __builtin_ctzll( aabbccdd_BCD ) / 8;
        u64 lo_lz = __builtin_ctzll( eeffgghh_BCD ) / 8;
        //if(aabbccdd_BCD==0)hi_lz=8;
        if(eeffgghh_BCD==0)lo_lz=8;//__builtin_ctzll(0) may cause bug on some CPU
        u64 lz = (aabbccdd_BCD > 0) ? hi_lz : (8 + lo_lz);
        e10 -= lz - 1;
        buf[0] = buf[lz];
        ...
        exp_pos = exp_pos - lz + 1 - (exp_pos - lz + 1 == 2 );
    }
    *(u64*)&buf[exp_pos] = ( -3<=e10 && e10<=e10_UP ) ? 0 : calc_f64_exp(e10); 
    return buf + exp_pos + (( -3<=e10 && e10 <= e10_UP ) ? 0 : (4 + (e10>=100||e10<=-100)) );
}