...
{
    unsigned long long vi = *(unsigned  long long*)&v;
    unsigned long long ieee_significand = vi & ((1ull<<52) - 1);
    unsigned long long ieee_exponent = (vi>>52) & 2047;
    ...
    if (regular) [[likely]] // branch
        ...
        k = (q * 315653 - 131237) >> 20;
    int h = q + (((-1 - k) * 217707) >> 16); // h --> range [-4,-1]
    ...
    u64 even = (c + 1) & 1;
    u128 cb = c << (h + 1 + offset);                    // h1 = h+1+offset = [-3,0]+offset ; offset-3 <= h1 <= offset;
    u128 hi128 = (cb * sig[0] + ((cb * sig[1]) >> 64)); // sig[0] : hi 64bit; sig[1] : low 64bit
    ...
    if (regular) [[likely]]                                                                          // branch
    {
        d = (half_ulp + even > dot_one) ? ten : d; // one = 0 ; cmov
    }
    else //[[unlikely]]
    {
        ...
        d = (half_ulp / 2 > dot_one) ? ten : d;// one = 0 ; cmov
    }
    d = (half_ulp + even > ~0 - dot_one) ? ten + 10 : d; // one = 10 ; cmov
    *dec = d;
    *e10 = k;
}